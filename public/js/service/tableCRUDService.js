...
import {addPopoverTriggerEl, EnableDragAndDrop} from "../Pattern/command/tableCommand.js";

export function saveToTableByName(tableName, table){
    let currentUser = firebase.auth()?.currentUser?.uid;
    if (currentUser){
    //let payload = stringifyWrapperTable(table);
    let payload = table.querySelector("tbody").innerHTML;
...
export function loadTableByName(tableName, table){
    ...
    if (currentUser) {
        getUserData(currentUser, tableName).then(response => {
            ...
            tbody.innerHTML = response.payload;
            ...
            new EnableDragAndDrop().execute({container: table.id})
